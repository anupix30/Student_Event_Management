...
async def handle_register(conn, data) -> dict:
    ...
    if role != 'student':
        return {'success': False, 'message': 'Only student registration is allowed here.'}
    ...
    if not name or not email or not password or not reg_no or not dept or not year:
    ...
    try:
        async with conn.cursor() as cur:
            ...
            """, (name, email, password, dept, reg_no, year))
            user_id = cur.lastrowid
            return {'success': True, 'user_id': user_id, 'role': 'student', 'name': name}
    except Exception as e:
...
async def handle_register_event(conn, data) -> dict:
    ...
    try:
        ...
        return {'success': False, 'message': 'Already registered or database error.'}
...
async def handle_fetch_history(conn, data) -> dict:
    ...
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            ...
            elif role in ('organiser', 'organizer'):
                await cur.execute(
                    "SELECT * FROM events WHERE organiser_id = %s ORDER BY id DESC",
                    (user_id,)
                )
                events = await cur.fetchall()
                for e in events:
        ...
        return {'success': False, 'message': 'Invalid role for history fetch.'}
    except Exception as e:
...
if __name__ == "__main__":
    # Fix for Windows asyncio + websockets bug (WinError 10014)
